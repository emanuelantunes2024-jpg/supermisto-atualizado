...
export function DetalheReceita({ receita }: { receita: Receita }) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid gap-5 app:grid-cols-[minmax(0,1.35fr)_minmax(0,1fr)] app:items-start">
        {/* 1. Capa */}
        <div className="relative order-1 aspect-[16/10] w-full overflow-hidden rounded-card bg-cream-200 app:col-start-1 app:row-start-1 app:aspect-[4/3]">
          <CapaReceita
            nome={receita.nome}
        ...
        {/* 3. Ingredientes, preparo e o resto do conteúdo */}
        <div className="cartao order-3 min-w-0 overflow-hidden app:col-start-1 app:row-start-2">
          <div
          ...
          >
            {ABAS.map((item) => (
              <button
                ...
                className={`shrink-0 border-b-2 px-3 py-3 text-[12.5px] font-semibold transition ${
                  aba === item.id
                ...
                {receita.equipamentos.map((equipamento) => (
                  ...
                  >
                    <Icone
                      nome="check"
                      tamanho={15}
                      className="shrink-0 text-money-500"
                    />
                    {equipamento}
                  </li>
                ))}
            ...
            {aba === "informacoes" && (
              <dl className="grid gap-3 sm:grid-cols-2">
                {[
                  {
                    rotulo: "Categoria",
                    valor: categoria?.nome ?? receita.categoria,
                  },
                  {
                    rotulo: "Subcategoria",
                    valor: receita.subcategoria ?? "—",
                  },
                  {
                    rotulo: "Tempo de preparo",
                    valor: formatarTempo(receita.tempoMinutos),
                  },
                  {
                    rotulo: "Dificuldade",
                    valor: ROTULO_DIFICULDADE[receita.dificuldade],
                  },
                  {
                  ...
                  {
                    rotulo: "Publicada em",
                    valor: new Date(receita.publicadaEm).toLocaleDateString(
                      "pt-BR",
                    ),
                  },
                ].map((info) => (
        ...
        {/* 2. Nome, informações e ações — no celular, logo abaixo da foto */}
        <div className="order-2 flex min-w-0 flex-col gap-4 app:col-start-2 app:row-start-1">
          <div>
            <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              {receita.paraVender && (
                <Selo tom="dinheiro">Boa para vender</Selo>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            >
              <Icone
                nome="coracao"
                tamanho={16}
                className={favorito ? "fill-current" : ""}
              />
            ...
            <button
              ...
              className="col-span-2 inline-flex items-center justify-center gap-2 rounded-xl border border-line bg-white px-3 py-2.5 text-[13px] font-semibold text-ink-soft transition hover:border-brand-300 hover:text-brand-600"
            >
            ...
            <div className="mb-2.5 flex items-center justify-between gap-3">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  value={rendimento}
                  onChange={(e) =>
                    setRendimento(Math.max(1, Number(e.target.value) || 1))
                  }
        ...
        {/* 4. Custos e preço sugerido */}
        <div className="order-4 min-w-0 app:col-start-2 app:row-start-2 app:sticky app:top-20">
